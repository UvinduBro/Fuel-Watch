  ...
  return (
    <div className="glass-panel p-4 flex flex-col sm:flex-row gap-4 justify-between items-center z-10 sticky top-4 shadow-2xl">
      <div className="relative w-full sm:w-96">
        ...
        {!isEditingLocation ? (
          <button 
            className="w-full text-left bg-muted/50 border border-border rounded-xl pl-10 pr-4 py-2.5 text-sm font-semibold transition-all hover:bg-muted hover:border-primary/30 text-foreground"
            onClick={() => { setIsEditingLocation(true); setTempLocation(customLocation); }}
          >
            {customLocation ? `${t("filter.searching")}: ${customLocation}` : `${t("filter.yourLocation")} • ${locationName}`}
          </button>
        ) : (
          <input 
            autoFocus
            ...
            className="w-full bg-background border border-primary/50 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-primary transition-all font-bold placeholder:text-muted-foreground/70"
            value={tempLocation}
      ...
      <div className="flex gap-2 w-full sm:w-auto">
        ...
        >
          ...
          {showOpenOnly ? t("filter.openOnly") : t("filter.allStations")}
        </button>
      </div>
    </div>
  );
